      ...
      <Form {...getFormProps(form)} action={action} className="space-y-2.5" >
        ...
        <div>
          <TextField
            ...
            defaultValue={lastResult?.initialValue?.passwordConfirmation.toString() ?? ''}
            isDisabled={isPending || isOauthPending}
            errorMessage={''}
          />
          <span id={fields.passwordConfirmation.errorId} className='text-sm text-red-500'>
            {fields.passwordConfirmation.errors && fields.passwordConfirmation.errors?.length > 1
